...
def execute(filters=None):
	...
	for project in data:
		project["total_tasks"] = frappe.db.count("Grant Task", filters={"grant_project": project.name})
		project["completed_tasks"] = frappe.db.count(
			"Grant Task", filters={"grant_project": project.name, "status": "Completed"}
		)
		project["overdue_tasks"] = frappe.db.count(
			"Grant Task", filters={"grant_project": project.name, "status": "Overdue"}
		)
...
def get_columns():
	return [
		...
		{"fieldname": "total_tasks", "label": _("Total Grant Tasks"), "fieldtype": "Data", "width": 120},
		{
			"fieldname": "completed_tasks",
			"label": _("Grant Tasks Completed"),
			"fieldtype": "Data",
			"width": 120,
		},
		{"fieldname": "overdue_tasks", "label": _("Grant Tasks Overdue"), "fieldtype": "Data", "width": 120},
	...
	return {
		"data": {
			...
			"datasets": [
				...
				{"name": "Total Grant Tasks", "values": total[:30]},
			],
		},
		"type": "bar",
...
def get_report_summary(data):
	...
	return [
		...
		},
		{
			"value": total,
			"indicator": "Blue",
			"label": _("Total Grant Tasks"),
			"datatype": "Int",
		},
		{
			"value": completed,
			"indicator": "Green",
			"label": _("Completed Grant Tasks"),
			"datatype": "Int",
		},
		{
			"value": total_overdue,
			"indicator": "Green" if total_overdue == 0 else "Red",
			"label": _("Overdue Grant Tasks"),
			"datatype": "Int",
		},
	]
