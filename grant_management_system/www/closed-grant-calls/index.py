...
def get_context(context):
    ...
    context.closed_grant_list = pagination.get('closed_grant_list')
...
def paginate(doctype, page=0):
    ...
    if(page):
        page = int(page)
        closed_grant_list = frappe.db.sql(query+f"""LIMIT {(page*page_length)-page_length}, {page_length};""", as_dict=True)
        ...
        if(count > page_length):
            ...
            pass
        closed_grant_list = frappe.db.sql(query+f"""LIMIT {page_length};""", as_dict=True)
    return {
        'closed_grant_list': closed_grant_list,
        'prev': prev,
        'next': next,
    }